...
# --- NEW HELPER: GET MATCHING SHEETS ---
def get_available_match_sheets():
    """
    Scans the workbook for sheets created by the matching algorithm.
    Criteria: Contains "Party" AND ("Matches" OR "Flow").
    """
    client = get_gspread_client()
    ...
    try:
        ...
        for ws in worksheets:
            title = ws.title
            # Filter for titles generated by the Admin Dashboard logic
            if "Party" in title and ("Matches" in title or "Flow" in title):
                match_sheets.append(title)
        return sorted(match_sheets)
    except Exception as e:
...
# ==========================
# TAB 3: PARTY EXCUSES (LINK ONLY)
# ==========================
with tab3:
    st.header("Recruitment Party Excuse Form")
    st.markdown("Submit an excuse for **one party at a time**. You must provide a **shareable link** to your proof.")
    ...
    if excuse_name and selected_party_clean:
        ...
        if sheet_excuses:
            ...
            if row_idx:
                ...
                # Pre-fill Reason (Col 5 / Index 4)
                if len(row_vals) > 4:
                    existing_reason = row_vals[4]
                
                # Pre-fill Link (Col 6 / Index 5)
                if len(row_vals) > 5:
                    existing_link = row_vals[5]
    ...
    with st.form(key='excuse_form'):
        ...
        # 3. Proof: Link Only
        st.write("---")
        st.markdown("**Proof of Excuse (Link Required)**")
        st.caption("Please upload your proof (image/screenshot) to Google Drive, Dropbox, or iCloud, then paste the **shareable link** below.")
        
        proof_link = st.text_input("Paste Shareable Link:", value=existing_link, placeholder="https://drive.google.com/...")
    ...
    if submit_excuse:
        ...
        if not excuse_name:
        ...
        elif not proof_link.strip() or "http" not in proof_link: 
            st.error("⚠️ You must provide a valid link (starting with http/https) to your proof.")
        else:
            sheet_excuses = get_sheet("Party Excuses")
            ...
            if sheet_excuses and sheet_mem:
                try:
                    ...
                    # Data Structure: Timestamp, Name, ID, Clean Party Name, Reason, PROOF LINK
                    row_data = [timestamp, excuse_name, member_id, selected_party_clean, excuse_reason, proof_link.strip()]
...
# ==========================
with tab6:
    st.header("View Party Matches")
    st.caption("Select a schedule below to view matches generated by the matching team.")
    ...
    if sheet_map:
        ...
        if sheet_matches:
            try:
                data = sheet_matches.get_all_values()
                if data:
                    ...
                    if not df_matches.empty:
                        if search_query:
                            mask = df_matches.astype(str).apply(lambda x: x.str.contains(search_query, case=False)).any(axis=1)
                            st.dataframe(df_matches[mask], use_container_width=True, hide_index=True)
                        else:
                            st.dataframe(df_matches, use_container_width=True, hide_index=True)
                    else:
                        st.info("This schedule sheet is empty.")
                else:
                    st.info("This schedule sheet is empty.")
            except Exception as e:
